...
truncate -s 0 $mountdir/swap/swapfile
# --> create empty swapfile
chattr +C $mountdir/swap/swapfile
btrfs property set $mountdir/swap/swapfile compression none
# --> disable btrfs compression for the swapfile
dd if=/dev/zero of=$mountdir/swap/swapfile bs=1M count=$swapsize
# --> change count value to wanted size of swap (8GB in this case)
chmod 0600 $mountdir/swap/swapfile
# --> only allow root to acces the swapfile
mkswap $mountdir/swap/swapfile
